self.onmessage = function(e) {
  const { imageData, kernel, targetChannel, strategy } = e.data;
  const { width, height, data } = imageData;
  
  const padImage = (data, width, height, strategy, kernelSize) => {
  ...
  const convolveChannel = (padded, kernel, channelOffset, width, height, pad) => {
    ...
    const result = new Uint8ClampedArray((width - 2 * pad) * (height - 2 * pad));
  ...
  const pad = 1;
  const padded = padImage(data, width, height, strategy, 3);
  const { width: paddedWidth, height: paddedHeight, data: paddedData } = padded;
  ...
  for (let ch of channelOffsets) {
    const convResult = convolveChannel(padded, kernel, ch, paddedWidth, paddedHeight, pad);
    ...
    for (let i = 0; i < data.length; i += 4) {
      for (let c = 0; c < 4; c++) {
        if (!channelOffsets.includes(c)) {
          resultData[i + c] = data[i + c];
        }
      }
    }
  }
  
  self.postMessage({ resultData, width, height });
};